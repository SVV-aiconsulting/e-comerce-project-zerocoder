...
## Возвраты кандидата 1.1

- Полный возврат создаётся из фактического сохранённого `receipt` успешного платежа.
- Частичный возврат в Django Admin задаётся индексами строк чека и количеством.
  Произвольная сумма без распределения по позициям отклоняется; итог вычисляет backend.
- Ожидающие и успешные возвраты резервируют количество и сумму, поэтому параллельные
  операции не могут превысить оплаченный остаток.
- UUID операции становится `Idempotence-Key`. Сетевой повтор продолжает ту же запись
  `Refund`; при сохранённом внешнем ID сначала выполняется GET у провайдера.
- Неопределённая операция старше 23 часов не создаётся заново автоматически и требует
  сверки менеджером до выхода за 24-часовое окно идемпотентности ЮKassa.
