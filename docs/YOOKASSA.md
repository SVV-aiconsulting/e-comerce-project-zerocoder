...
Для платежа формируется `receipt`: позиции заказа, отдельная строка доставки, email либо
телефон покупателя, НДС, тип расчёта и предмет расчёта. Без email/телефона создание
платёжной ссылки блокируется, потому что кассовые данные неполны. `Refund` хранит отдельную
операцию и свой idempotence key; полный и частичный возвраты создаются через API.

Тестовый режим проверяет API-сценарий и данные чека, но не отправляет их в реальную кассу
или ОФД и не формирует юридически значимый чек.
