...
используется только тестовый магазин. Тестовые платежи и данные чека проходят весь
API-сценарий, однако тестовая касса имитирует фискализацию: она не передаёт данные в ОФД
и не отправляет письмо покупателю. Реальные платежи, фискальные чеки и возвраты не
создаются, пока не выполнен production-чек-лист ниже.
...
email обязателен: в коммерческом контуре ОФД использует его для электронного чека.
Телефон и email, введённый пользователем уже на платёжной странице, не могут заменить
контакт в ранее созданном API-запросе. В Telegram бот запрашивает email до создания
ссылки; website требует email при выборе «Картой онлайн».
...
Тестовый режим проверяет API-сценарий и структуру данных чека, но не заменяет настройку
54‑ФЗ в личном кабинете. После включения режима проверки чеков статус «Чек доставлен в
облачную кассу» означает только, что тестовая касса приняла данные. Это **не** статус
доставки письма: в sandbox ЮKassa имитирует работу кассы, не передаёт данные в ОФД и
намеренно не отправляет email покупателю. Поэтому отсутствие письма при успешном
тестовом чеке — ожидаемый результат, а не ошибка WebMarket.

В коммерческом контуре подключите «Чеки от ЮKassa» или совместимую онлайн-кассу,
выберите сценарий «Платёж и чек одновременно», заключите договор с ОФД и проверьте
доставку на реальный email. При отсутствии письма после production-платежа сначала
проверяют адрес и папку «Спам», затем повторно отправляют уже сформированный чек из
кабинета кассы/ОФД; новый платёж или дубликат продажи для этого не создают.
...
Этот раздел не входит в обязательные шаги текущего учебного MVP. Он становится
обязательным только перед переключением с тестового магазина на коммерческий
контур; незаполненные налоговые параметры не блокируют sandbox-тестирование и
демонстрацию реализованного сценария.
