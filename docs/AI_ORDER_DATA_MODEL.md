...
Customer 1 -------- N OrderDraft 1 -------- N OrderDraftItem N -------- 0..1 Product
   ...
   +------ N InboundEvent N -------- 0..1 OrderDraft
                              |
                              +------ N AIExtractionRun
                              +------ 0..1 AssistantTurn 1 ------ N AssistantToolCall
                              +------ N AssistantMessage
...
- фактический ответ ассистента хранится отдельно от текущего состояния черновика;
- каждый mutating tool call имеет idempotency key и аудируемый результат.

### 2.1 Agent runtime, добавленный 04.09.2026

- `AssistantMessage` — неизменная история ролей `user/assistant`, тип ответа и URL
  действия, фактически показанные клиенту;
- `AssistantTurn` — состояние хода, модель, число model/tool calls, токены, задержка
  и код ошибки;
- `AssistantToolCall` — имя функции, типизированные аргументы, результат, статус,
  задержка и ключ идемпотентности.

Старые `AIExtractionRun` и `Clarification` сохранены для совместимости и аудита
ранее обработанных данных, но новый основной workflow не создаёт repair-запуски и
не эскалирует обычные уточнения автоматически.
