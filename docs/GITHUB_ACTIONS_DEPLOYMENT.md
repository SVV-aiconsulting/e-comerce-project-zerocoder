...
Пока все обязательные VPS Secrets не созданы, deploy-job намеренно помечается
`skipped`: тесты и публикация образов остаются зелёными, а попытки подключиться к
пустому адресу сервера не выполняются.
