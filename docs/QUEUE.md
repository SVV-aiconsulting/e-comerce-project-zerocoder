...
- Обработка сериализуется PostgreSQL advisory lock по паре `channel + external_user_id`;
  одновременно менять одну корзину два диалога не могут.
- Вызов LLM и сетевые операции не держат транзакцию события открытой. Lease
  проверяется до и после внешнего вызова и перед каждым сохранением ответа/tool audit.
...
- Dispatcher также подбирает просроченный `processing`; исчерпавшее лимит событие
  переводится в `failed` и остаётся в очереди исключений менеджера.
